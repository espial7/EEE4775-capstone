 ...
 *
 * What you do:
 *   1. Pick which signaling path your "real" handler uses (or both).
 *   2. Run >= 50 button presses idle (WITH_LOAD 0); measure the
 *      GPIO-19-to-task-start latency for each path.
 *   3. Flip WITH_LOAD to 1 to bring App 2's four tasks online, then re-run the
 *      >= 50 presses and re-measure. Report idle vs loaded.
 *   4. Induce a failure: comment out portYIELD_FROM_ISR(), document what changes.
 *   5. Defend in README.
 *
...
static void btn_task_sem(void *arg)
{
    for (;;) {
        if (xSemaphoreTake(btn_sem, portMAX_DELAY) == pdTRUE) {
            ...
            ESP_LOGI(TAG, "[sem] E-STOP pressed —> assert safe state (press #%lu)  latency=%lld us (max=%llu)",
...
static void btn_task_notif(void *arg)
{
    for (;;) {
        ...
        ESP_LOGI(TAG, "[notif] E-STOP press #%lu  latency=%lld us (max=%llu) "
                      "(notif count=%lu)",
                 (unsigned long)presses_observed,
                 (long long)lat,
...
void app_main(void)
{
    ...
    esp_task_wdt_deinit();   // Disable the Task Watchdog
